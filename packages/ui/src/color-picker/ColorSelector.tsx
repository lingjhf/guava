...
export interface GColorSelectorProps {
  color: Color
  size: Size
  sliderSize: Size
...
export const GColorSelector = (props: Partial<GColorSelectorProps>) => {
  let colorSelectorRef: HTMLElement
  const setColorSelectorRef = (el: HTMLElement) => (colorSelectorRef = el)

  const defaultProps = mergeProps<[GColorSelectorProps, ...Partial<GColorSelectorProps>[]]>(
    {
      color: Color().hsv(),
